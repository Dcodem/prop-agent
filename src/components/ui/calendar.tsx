...
import { Input } from "@/components/ui/cal-input";
...
const filterPresets = (obj: Record<string, any>, search: string) => {
  ...
  const filtered = Object.fromEntries(
    Object.entries(obj).filter(([, value]) => {
      ...
      return searchWords.every((word: string) => keyLower.includes(word));
    })
  );
...
const formatDateRange = (start: Date, end: Date, timezone: string) => {
  ...
  if (sameMonth) return `${formatMonth(start)} ${formatDay(start)} - ${formatDay(end)}`;
  if (sameYear) return `${formatMonth(start)} ${formatDay(start)} - ${formatMonth(end)} ${formatDay(end)}`;
...
const CalendarCombobox = ({ stacked, compact, value, onChange, presets, presetIndex }: CalendarComboboxProps) => {
  ...
  const onClick = (v: any) => {
    ...
    setCurrentPreset(v);
    onChange({ start: v.start, end: v.end });
    setIsOpen(false);
  };
      ...
      >
        <ul className="p-2 border-r border-r-gray-200">
          {Object.entries(filteredPresets).length > 0
            ? Object.entries(filteredPresets).map(([key, v]) => (
                <li
                  key={key}
        ...
        {!compact && (
          <div className="p-4 pr-[30px]">
            ...
            <div className="mt-2 flex flex-wrap gap-1">
              {typeFixedTimes.map((v) => (
                <button key={v.text} className="font-mono text-[13px] text-gray-1000 px-1.5 h-5 inline-flex items-center bg-accents-2 border-none rounded cursor-pointer">
                  {v.text}
                </button>
              ))}
            </div>
...
export const Calendar = ({
  ...
  const [startTime, setStartTime] = useState(formatInTimeZone(startOfDay(value?.start || new Date()), selectedTimezone, "HH:mm"));
  const [endDate, setEndDate] = useState(formatInTimeZone(value?.end || new Date(), selectedTimezone, "MMM dd, yyyy"));
  const [endTime, setEndTime] = useState(formatInTimeZone(endOfDay(value?.end || new Date()), selectedTimezone, "HH:mm"));
  ...
  return (
    <div className="relative">
      <div className={clsx(presets && "flex", presets && stacked && "flex-col", compact && "w-[220px]")}>
        {presets && (
          <div>
            <CalendarCombobox stacked={stacked} compact={compact} presets={presets} value={value} onChange={onChange} presetIndex={presetIndex} />
          </div>
        )}
          ...
          <div className="relative">
            ...
            >
              <div className="truncate pr-4">
                {value?.start && value?.end ? formatDateRange(value.start, value.end, selectedTimezone) : "Select Date Range"}
              </div>
            </Button>
                ...
                {daysArray.map((d) => {
                  ...
                  const isInRange =
                    ...
                    ((value.end && isWithinInterval(d, { start: value.start, end: value.end })) ||
                      (hoverDate && isWithinInterval(d, { start: value.start, end: hoverDate })));
                  const isAllowedDate = (minValue ? d >= minValue : true) && (maxValue ? d <= maxValue : true);
                  ...
                  return (
                    <div
                      ...
                      className={clsx(
                        ...
                        isSameMonth(d, currentDate) && isAllowedDate ? "bg-background-100 text-gray-1000" : "bg-background-100 text-gray-700",
                    ...
                    >
                      <div
                        className={clsx(
                          ...
                          !isStart && !isEnd && !currentHover && !isToday(d) && isAllowedDate && "hover:text-gray-1000 hover:border hover:border-gray-alpha-500",
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-1">
                    <div className={showTimeInput ? "col-span-2" : "col-span-3"}>
                      <Input size="small" value={startDate} onChange={(v) => setStartDate(v)} error={startDateError} />
                    </div>
                    {showTimeInput && <Input size="small" value={startTime} onChange={(v) => setStartTime(v)} error={startTimeError} />}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-1">
                    <div className={showTimeInput ? "col-span-2" : "col-span-3"}>
                      <Input size="small" value={endDate} onChange={(v) => setEndDate(v)} error={endDateError} />
                    </div>
                    {showTimeInput && <Input size="small" value={endTime} onChange={(v) => setEndTime(v)} error={endTimeError} />}
                  </div>
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <div className="font-medium flex flex-col">
                  <Button type="secondary" size="small" suffix={<span className="mt-1 text-xs">↵</span>} onClick={onApply}>
                    Apply
                  </Button>
                </div>
                <div className="w-fit self-center">
                  <Select size="xsmall" variant="ghost" options={timezones} value={selectedTimezone} onChange={(event) => setSelectedTimezone(event.target.value)} />
                </div>
              </div>
            </div>
          </div>
